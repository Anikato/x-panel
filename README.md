...
```bash
# 自定义端口 + 安全入口（推荐）
curl -sSL https://xpanel.qm.mk/install-online.sh \
...
# 自定义更新服务器
bash install-online.sh --update-url https://xpanel.qm.mk \
...
| `--version, -v <版本>` | 安装指定版本 |
| `--update-url <URL>` | 自定义更新服务器（默认 `https://xpanel.qm.mk`） |
...
```bash
curl -sSL https://xpanel.qm.mk/install-online.sh | bash -s -- --uninstall --yes
```
