...
help:
	...
	@echo "  NEZHA_AGENT_BINARY  - 调用 package 时必填：已校验的定制兼容 Agent 二进制路径"
...
# 本地打包验证（需提供已校验的定制兼容 Agent 二进制；不联网、不选 latest）
package: build
	@echo ">>> 打包发布..."
	@if [ -z "$(NEZHA_AGENT_BINARY)" ]; then \
		echo "error: NEZHA_AGENT_BINARY is required (path to verified customized Agent binary)" >&2; \
		exit 1; \
	fi
	...
	# 捆绑已校验的定制兼容 Agent + 许可证/声明 + unit（0644/0755）
