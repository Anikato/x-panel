...
- [x] 源码准备 X-Panel `v0.8.3-20260924.2`：证书服务 Token 留空不再更换，并记录其他面板的拉取和失败
- [ ] GitHub `Build & Release` 打出 `v0.8.3-20260924.2` 正式包

### 关键决策

- 已有 `X-Cert-Token` 保持有效；只有填写新 Token 并保存才更换
- 来源名称用可选请求头 `X-Cert-Client`，旧面板不带这个头也能继续同步
